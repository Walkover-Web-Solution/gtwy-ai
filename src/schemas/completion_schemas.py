...
# Keep in sync with src.configs.constant.service_name
...
class ConfigurationModel(BaseModel):
    # Allow unknown keys so new model params from the UI/DB do not break validation.
    model_config = ConfigDict(extra="allow")
...
class CompletionRequest(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_known_service(self) -> "CompletionRequest":
        # Defense in depth: Literal already restricts, but also guard against drift
        # if callers construct models without going through field validation.
