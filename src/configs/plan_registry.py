...
needs no deploy. That is the whole point — it replaces the `free_tier` boolean,
which was spread across hundreds of modelconfigurations documents with no
reachable admin API. Seeded once per environment by the Node repo's
scripts/bootstrapBilling.js; edited thereafter through PUT /api/billing-plans.
    ...
    try:
        ...
        if not plan_registry_document:
            logger.error(
                ...
                "(Node repo: node scripts/bootstrapBilling.js)."
            )
