"""Bridge between sandboxed AI-generated code and the existing tool
...
class UnknownTool(Exception):
    """The sandboxed script referenced a tool name that isn't configured for this bridge."""
...
async def execute_single_tool(name, args, ctx):
    ...
    needed here — the sandboxed script already sequences its own calls).
...
def make_call_tool(ctx, invocation_log):
    """Build the single async hook exposed to sandboxed code. Every
    ...
    No call-count limit here — sandbox.py's own loop-iteration budget and
    wall-clock timeout are what bound a runaway script.
    """
