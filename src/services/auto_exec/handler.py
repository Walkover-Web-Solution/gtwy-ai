...
for where that tool gets registered). Validates and runs the generated code
in the sandbox, using the exact tool-dispatch primitives already used by the
normal tool-calling loop.
"""

from globals import logger
from src.services.auto_exec.sandbox import SandboxRejected, run_sandboxed
...
async def run_generated_plan(args, service_instance):
    ...
    try:
        result_value = await run_sandboxed(code, call_tool)
        return {
