...
async def _lago_get(path: str, params: dict) -> dict:
    """GET one Lago collection with the billing session settings above.

    Raises Exception on any non-2xx answer; the message carries the status and
    the first 300 bytes of the body, which is all the callers log.
    """
    url = f"{Config.LAGO_API_URL.rstrip('/')}/{path}"
    headers = {
    ...
    async with aiohttp.ClientSession(timeout=_LAGO_TIMEOUT) as session:
        async with session.get(url, headers=headers, params=params) as response:
            if response.status >= 300:
                ...
                raise Exception(f"Lago {path} lookup failed ({response.status}): {body[:300]}")
...
async def get_wallet_balance(org_id: str) -> Decimal:
    ...
    truth (§4).
    """
    payload = await _lago_get("wallets", {"external_customer_id": org_id})
...
async def get_subscription_plan_slug(org_id: str) -> str | None:
    ...
    plan cache is filled from. Same tight timeout and no-retry session as the
    balance lookup — this sits on the request path whenever the cache misses,
    so it must fail fast rather than stall a user's request.

    Returns None (rather than raising) for every "we don't know" case: no
    subscription, an unrecognised plan code. Lago unreachable / non-2xx raises,
    which the caller (billing_utils.get_org_plan) turns into the default plan.
    """
    payload = await _lago_get("subscriptions", {"external_customer_id": str(org_id)})
