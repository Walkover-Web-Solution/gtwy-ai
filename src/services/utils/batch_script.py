                            ...
                            if item_usage and model:
                                ...
                                if item_input_tokens > 0 or item_output_tokens > 0:
                                    try:
                                        ...
                                        item_usage['cost'] = cost_breakdown.get('total_cost', 0) * 0.5
                                    except Exception as cost_err:
                                        logger.error(f"Error calculating cost for batch item: {cost_err}")
                                        item_usage['cost'] = 0
                        ...
                        if webhook.get('url') is not None:
                            try:
                                webhook_response = await sendResponse(
                                    response_format, data=formatted_results, success=has_success
                                )
                            ...
                            if org_id and model:
                                ...
                                if input_tokens > 0 or output_tokens > 0:
                                    try:
                                        ...
                                        logger.error(f"Error calculating cost for message {message_id}: {cost_error}")
