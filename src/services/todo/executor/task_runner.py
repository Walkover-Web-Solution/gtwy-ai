...
async def _read_connected_agent_response(
    ...
    Forwards tool events to the parent streamer when the response is an SSE
    iterator; delta/reasoning are accumulated server-side but NOT forwarded
    — for A2A the deltas are the internal JSON envelope and shouldn't leak
    into the UI. The caller emits a structured task_completed /
    task_waiting_for_user event with the parsed envelope instead.

    Falls back to body/dict shapes if the service did not stream despite
    stream=True."""
...
async def _run_connected_agent(
    ...
    if streamer:
        await streamer.emit_delta(json.dumps({
            "event": "task_agent_thinking",
            "task_id": task_id,
            "agent_id": assigned_agent,
            "agent_name": (primary_config or {}).get("name") or assigned_agent,
        }))
