...
async def run_stream_and_collect(generator, streamer):
    ...
    async for delta in generator:
        ...
        if delta.get("error"):
            ...
            await streamer.emit_error(delta["error"])
            break
        ...
        if delta.get("tool_calls") is not None:
            ...
            for tc in (final_tool_calls or []):
                ...
                args_raw = tc.get("input") or tc.get("function", {}).get("arguments", "")
                try:
                    args = json.loads(args_raw) if isinstance(args_raw, str) else (args_raw or {})
                except Exception:
                    args = {}
                await streamer.emit_tool_call(name=name, args=args, call_id=call_id)
        if delta.get("usage"):
